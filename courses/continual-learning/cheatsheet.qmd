...
**Atlas** — the Omega rule (memorize a window, §3.2 Eq. 9) + a Muon-style step (§5, Eqs. 32–33):
...
- Inner objective $\lVert\mathcal M(\mathbf k)-\hat{\mathbf v}\rVert^2$; inner optimizer = **DGD + weight decay** (M6 — tokens are correlated); the memory is a residual MLP. $\mathbf q_t=\mathbf x_tW_q$ is the only non-adaptive projection (§8.1).
